...
import { AdvectionQuality, ColourSeed } from '../../math/vector-field/advection-colour/worker-setup-advection-colour';
import { AtmosphereDescriptor } from '../../math/vector-field/atmosphere-field/types';

export function createAtmosphereColourSeeds(
    ...
    const bandMapper = ColourMapper.fromString(descr.bandGradient.supportPoints, descr.bandGradient.easing);
    ...
    for (let row = 0; row < bandSeedRows; row++) {
        for (let col = 0; col < bandSeedCols; col++) {
            ...
            seeds.push({ x, y, colour: bandMapper.mapLooped(t, 0.5, 0.5) });
        }
    }

    // Vortex seeds — filled circle of seeds within each vortex radius
    const vortexSeeds: ColourSeed[] = [];
    for (const vortex of descr.vortices) {
        if (vortex.colour === null) continue;
        ...
        while (placed < vortexSeedsPerVortex) {
            ...
            vortexSeeds.push({ x, y, colour: vortex.colour });
            placed++;
        }
    }
