        ...
        for (let row = 0; row < this.grid.height; row++) {
            for (let col = 0; col < this.grid.width; col++) {
                ...
                this.setPixel(
                    ...
                    index,
                    blender.blend(
                        (isNaN(magnitude)) ? fallbackColour : colourMapperMagnitude.mapClamped(magnitude, this.config.data.gradientMagnitude.scaling, this.config.data.gradientMagnitude.offset),
